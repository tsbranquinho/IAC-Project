...
SP_inicial_principal:                 
	...
	STACK 100H						  ; espaço reservado para o processo de controlo
SP_inicial_controlo:
...
sondas_ativas:
	LOCK 0							  ; sonda esquerda
	LOCK 0							  ; sonda central
	LOCK 0							  ; sonda direita
...
ativa_tecla_C:
	PUSH R1
	MOV  [tecla_C_carregada], R1
	POP  R1
	RET

ativa_tecla_D:
	PUSH R1
	MOV  [tecla_D_carregada], R1
	POP  R1
	RET
...
controlo:
	MOV R0, [tecla_C_carregada]
	CMP R0, 1
	JZ  comeca_jogo

	MOV R0, [tecla_0_carregada]
	CMP R0, 1
	JZ  ativar_sonda_esquerda

	MOV R0, [tecla_1_carregada]
	CMP R0, 1
	JZ  ativar_sonda_central
	
	MOV R0, [tecla_2_carregada]
	CMP R0, 1
	JZ  ativar_sonda_direita

	MOV R0, [tecla_D_carregada]
	CMP R0, 1
	JZ  estado_pausa

	JMP controlo

ativar_sonda_esquerda:
	MOV R0, [estado_jogo]
	CMP R0, 1
	JZ  sonda_esquerda_jogo
	JMP controlo

sonda_esquerda_jogo:
	PUSH R1
	MOV  [sondas_ativas], R1
	POP  R1
	JMP controlo

ativar_sonda_central:
	MOV R0, [estado_jogo]
	CMP R0, 1
	JZ  sonda_central_jogo
	JMP controlo

sonda_central_jogo:
	PUSH R1
	MOV  [sondas_ativas + 2], R1
	POP  R1
	JMP controlo

ativar_sonda_direita:
	MOV R0, [estado_jogo]
	CMP R0, 1
	JZ  sonda_direita_jogo
	JMP controlo

sonda_direita_jogo:
	PUSH R1
	MOV  [sondas_ativas + 4], R1
	POP  R1
	JMP controlo
...
pausa_jogo:
	...
	DI3
	DI
	MOV  R11, 2						   ; estado de jogo pausado
	MOV  [estado_jogo], R11
	MOV  [APAGA_AVISO], R1			   ; apaga o aviso de nenhum cenário selecionado (o valor de R1 não é relevante)
...
retoma_jogo:
	MOV  R11, 1						   ; estado de jogo retomado
	MOV  [estado_jogo], R11
...
setup:
	...
	MOV  R11, 1					      ; estado de jogo começado
	MOV  [estado_jogo], R11			  ; estado de jogo guardado na memória
	MOV  [APAGA_AVISO], R1			  ; apaga o aviso de nenhum cenário selecionado (o valor de R1 não é relevante)
    MOV  [APAGA_ECRÃ], R1			  ; apaga todos os pixels já desenhados (o valor de R1 não é relevante)
	MOV  R1, 0                        ; vídeo número 0
